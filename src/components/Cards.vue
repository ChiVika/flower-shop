<template>
    <div class="card">
        <h3 class="card__title" v-if="!isSalePage">Акции</h3>
        <h3 class="card__title1" v-if="isSalePage">Акции</h3>
        <div class="card__container">
            <div class="card__item" v-for="product in products" :key="product.id" >
                <div class="card__content" @click="clickEnter(product.id)">
                    <div class="card-item__picture">
                        <img :src="product.image" alt="dd" class="card-item__img">
                    </div>
                    <h3 class="card-item__title">{{ product.title }}</h3>
                    <!-- <button class="card-item__more" >Читать подробнее</button> -->
                    <p class="card-item__old" v-if="product.old !== null">{{ product.old }} р</p>
                    
                </div>
                <div class="card-item__forUser" v-if="product.old !== null">
                        <p class="card-item__price">{{ product.price }} р</p>
                        <button class="card-item__btn" @click="addToBasket(product)">В корзину</button>
                    </div>
                <div class="card-item__forUser card-item__forUser--mt30" v-if="product.old === null">
                    <p class="card-item__price">{{ product.price }} р</p>
                    <button class="card-item__btn" @click="addToBasket(product)">В корзину</button>
                </div>
            </div>
        </div>
        <button class="card__more" v-if="!AllSales">
            <RouterLink to="/sales" class="card__button1">Смотреть больше</RouterLink>
        </button>
        <button class="card__go-to" v-if="isSalePage">
            <RouterLink to="/shop" class="card__button-catalog" v-if="isSalePage">Перейти к каталогу</RouterLink>
        </button>
    </div>
...
export default {
    ...
    setup(props) {
        ...
        const products = computed(() => {
            if (props.AllSales) {
                return saleStore.getAllSales;
            } else {
                return saleStore.getSalesEight;
            }
        });
        return {
            products,
